% Вывод значений m в зависимости от величины выборки (чтобы понять, на
% какой минимальной выборке можно получить адекватное значение)
...
for i=index:-1:1
    ...
    X0 = X0(1+index-i:1+index+i);     %
    Y0 = Y0(1+index-i:1+index+i);     % подрезка векторов для исследования эффективности
...
%     disp(sprintf('A=%d',A));
%     disp(sprintf('B=%d',B));
      disp(sprintf('m=%d на выборке из %d элементов',m,(1+index+i)-(1+index-i)+1));
%     disp(sprintf('r=%d',r));
% 
